...
@Slf4j
@Component
public class JedisLuaDistribute {
    
    private static String LOCK_PREFIX = "JedisLuaDistribute_";
    ...
    public void lockJob() throws InterruptedException {
        String lock = LOCK_PREFIX + "NXEXLOCK";
...
//                String value = new String(get(lock));
                ...
                log.info("JedisLuaDistribute get lock fail ,lock belong to:{}", value);
                return;
            } else {
                //获取锁成功
                log.info("JedisLuaDistribute start lock success");
                Thread.sleep(5000);
            }

        } catch (Exception e) {
            log.error("JedisLuaDistribute lock error", e);
        } finally {
            if (lockRes) {
                releaseLock(lock,getHostIp());
                log.info("JedisLuaDistribute release lock success");
            }
        }
